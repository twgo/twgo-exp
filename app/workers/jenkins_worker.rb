class JenkinsWorker
  ...
  def ci_success_exp_git(exp_name)
    ...
    (1..all_build_counts).each do |x|
      uri = URI.parse("https://#{ENV['CI_HOST']}/job/#{exp_name}/#{x}/api/json")
  ...
  def docker_id(exp_name, id, result)
    if result=='SUCCESS'
      a = open("https://#{ENV['CI_HOST']}/job/#{exp_name}/#{id}/docker/", http_basic_authentication: [ ENV['CI_ID'], ENV['CI_PWD'] ]) {|f| f.read } .split
      a[(a.index('Id:</b>')+1)]
    else
      'FAILURE'
    end
  ...
  def exp_rate(exp_name, id, status)
    if status=='FAILURE'
      ...
      result = open("https://#{ENV['CI_HOST']}/job/#{exp_name}/#{id}/consoleText", http_basic_authentication: [ ENV['CI_ID'], ENV['CI_PWD'] ]) {|f| f.read }
      tri4_no_si result
    else
      888
    end
